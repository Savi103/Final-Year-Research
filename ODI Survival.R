...
plot(dfbetaresidual.ODI[,2], ylab = "Remaining Overs")
plot(dfbetaresidual.ODI[,3], ylab = "Wickets")
...
coeff.remaining.overs = summary(coxph.fit.ODI)$coefficients[5,1]
coeff.wickets = summary(coxph.fit.ODI)$coefficients[6,1]
...
#Return the c value
ODI.survival.estimate <- function(good, poor, satisfied, very.good, remain.overs, wickets, baseline){
  est <- coeff.good*good + coeff.poor*poor + coeff.satisfied*satisfied + coeff.very.good*very.good + coeff.remaining.overs*remain.overs + coeff.wickets*wickets
  exp.est <- exp(est)
...
for(i in 1:nrow(ODI.notout)){
  ...
  c <- ODI.survival.estimate(ODI.notout$Good[i], ODI.notout$Poor[i], ODI.notout$Satisfied[i], ODI.notout$`Very Good`[i], ODI.notout$`Remaining Overs`[i], ODI.notout$Wickets[i], ODI.notout$baseline.surv[i])
  c.vec <- c(c.vec, c)
}
...
plot(dfbetaresidual.ODI1[,2], ylab = "Remaining Overs")
plot(dfbetaresidual.ODI1[,3], ylab = "Wickets")
plot(dfbetaresidual.ODI1[,4], ylab = "Runs")
plot(dfbetaresidual.ODI1[,5], ylab = "Career SR")
...
coeff.remaining.overs = summary(coxph.fit.ODI1)$coefficients[5,1]
coeff.wickets = summary(coxph.fit.ODI1)$coefficients[6,1]
coeff.final.runs = summary(coxph.fit.ODI1)$coefficients[7,1]
coeff.strike.rate = summary(coxph.fit.ODI1)$coefficients[8,1]
...
#Return the c value
ODI.survival.est <- function(good, poor, satisfied, very.good, remain.overs, wickets, final.runs, strike.rate, baseline){
  est <- coeff.good*good + coeff.poor*poor + coeff.satisfied*satisfied + coeff.very.good*very.good + coeff.remaining.overs*remain.overs + coeff.wickets*wickets + coeff.final.runs*final.runs + coeff.strike.rate*strike.rate
  exp.est <- exp(est)
  survival <- baseline^(exp.est)
  return(exp.est)
}
...
for(i in 1:nrow(ODI5)){
  ...
  c <- ODI.survival.est(ODI5$Good[i], ODI5$Poor[i], ODI5$Satisfied[i], ODI5$`Very Good`[i], ODI5$`Remaining Overs`[i], ODI5$Wickets[i], ODI5$`Final Runs`[i], ODI5$`Career SR`[i], ODI5$baseline.surv.y[i])
  c.vec <- c(c.vec, c)
}
